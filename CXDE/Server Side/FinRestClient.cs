        ...
        public string SendHTTPRequest(string uri, string userName, string password, string method, string xmlParam)
        {
            ...
            if (restClient == null)
            {
                    if (_FinAgent._agentInformation.Ssl)
                    {
                        ...
                        if (_FinAgent._agentInformation.HttpConnectionType == null)
                            return result + "Http Connection Type is undefined";
                        else if (_FinAgent._agentInformation.HttpConnectionType.Equals("Ssl3"))
                            System.Net.ServicePointManager.SecurityProtocol = System.Net.SecurityProtocolType.Ssl3;
                        else if (_FinAgent._agentInformation.HttpConnectionType.Equals("Tls"))
            ...
            if (request != null)
            {
                ...
                {
                        _FinAgent.FireLogMessage("Error in rest communication, response is:" + response.Content);
                        _FinAgent.FireLogMessage("Error in rest communication, response Exception is:" + response.ErrorException);
                }
            }
            }
            catch (Exception)
